...
include_once 'bus_library/APICaller.php';
...
if(isset($_POST)){
	...
	$journey_date=date("Y-m-d",strtotime($_POST['datepicker']));
	$triptype=1;
}
...
// $ress=$obj->getBookingInfoByPaystatus('1');
// exit;
...
if(isset($_GET['prev']))
{
	$fromStation=$_GET['fromCity'];
	$triptype=1;
	$toStation=$_GET['toCity'];
	$date=$_GET['doj'];
...
$resultbus=getAvailableBuses($fromStation,$toStation,$journey_date);
/* ETS RESULT END */
$_SESSION['user']['apiAvailableBuses']=$resultbus['apiAvailableBuses'];
$_SESSION['user']['trips']=$trips=$resultbus['apiAvailableBuses'];
...
<ul class="searchAvail cf">
    <li class="leavego" id="origin_click">
 <input type="text" autocomplete="off" class="searchPlace move-up-hide" title="Origin" id="originid" name="ter_from">
...
<span class="err_msg" id="err" style="font-size:13px; font-weight:bold;color:#fff;">
  <span class="togglebtn">
    ...
    <li class="leavego">
<input type="text" autocomplete="off" title="Destination" class="searchPlace move-up-hide" id="destinationid" name="tag" > 
...
<span class="err_msg" id="err1" style="font-size:13px; font-weight:bold;color:#fff;">
    ...
    <input id="datepicker" name="datepicker" type="text" class="calendarInput move-up-hide" >
...
<span class="err_msg" id="err2" style="font-size:13px; font-weight:bold;color:#fff;">
    ...
    <input id="datepicker1" name="datepicker1" type="text" class="calendarInput move-up-hide" >
...
if($(this).parents('li').find('label.db').hasClass('move-up')){
	 if($(this).val()=="" && $(this).hasClass('move-up-hide')){
...
$(document).ready(function (){
$('body #origin_click').on('click','.tt-suggestion',function(){
$('#destinationid').focus();
