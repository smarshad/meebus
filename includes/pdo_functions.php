...
class User
{
	...
	function getBookingInfoByPaystatus($data)
	{
		...
		$stmt=$this->con->prepare("SELECT * FROM bookinginfo WHERE pay_status=".$data);
		$stmt->execute();
	...
	{
		$stmt = $this->con->prepare("SELECT balance FROM user_login WHERE id=?");
	  	$stmt->execute($data);
		return $stmt->fetchColumn();
	}	
	function addBookerDetail($book_detail)
	{
		//print_r($book_detail);
		try {
		//$stmt = $this->con->prepare("INSERT INTO book_bus_tickets (TentativeBooking_oneway,scheduleId,apitype,mode,travelDate,fromStationId,toStationId,boardingInfo,emailId,mobileNbr,lead_pax_name,passenger_name,passenger_age,passenger_sex,passenger_seat,no_of_seat,booked_on,status,total_fare,busInfo,blockRequest,oneway_fare,cancellationDescList,partialCancellation,DepartureTime,ArrivalTime,TravelHours,session_id_b,promocode,offerAmount,booking_type,payment_gateway,paidAmount,alternateno,dateOfIssue) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
		$stmt = $this->con->prepare("INSERT INTO book_bus_tickets (
		TentativeBooking_oneway,
		scheduleId,
		apitype,
		mode,
		travelDate,
	...
	function getTicketDetail($ref_id,$status)
	{
		...
		$stmt = $this->con->prepare("SELECT * FROM book_bus_tickets WHERE session_id_b=? AND status=?");
		$stmt->execute($data);
		$count=$stmt->rowCount();
		if($count==1)
		{
