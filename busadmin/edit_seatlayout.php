...
mysql_query("DELETE FROM bus_structure WHERE bus_id=".$inbus);
	...
	mysql_query("INSERT INTO bus_structure (bus_id,position,seat_no) VALUES ($inbus,$cc,'$exp[$ii]')") or die(mysql_error());
		...
		$exp2=explode(',',$imp2);
				...
				//echo "update  businfo set Bus_structure='$stridd' where Bus_id='$inbus'"; exit;
				
			mysql_query("update businfo set Bus_structure='$stridd' where Bus_id='$inbus'");	
			
			header("location:busDetails.php?sp_id=$_SESSION[SP_id]");
...
<td>
<select name="bus_seatlayout" onchange="chg_structure(this.value)"><option selected="selected">Select Type</option>
<?php
$str_sql=mysql_query("SELECT * FROM busstructuretypes WHERE structureStatus = 1 and structureID='$structureid' ORDER BY structureID ");
...
?>
<option value="<?php echo $strtypes['structureID']; ?>" <?php if($bus->Bus_structure == $strtypes['structureID']) { ?> <?php } ?>><?php echo $strtypes['structureType']; ?></option>
<?php } ?>
</select>
</td>
<td>
