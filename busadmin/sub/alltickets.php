...
//echo "SELECT bookinginfo.Ticket_ID,bookinginfo.travelling_date,bookinginfo.SP_id,bookinginfo.Bus_id,booker_details.Booker_name FROM bookinginfo,serviceprovider_info,businfo,booker_details WHERE bookinginfo.Ticket_ID!='' AND  bookinginfo.Ticket_ID=booker_details.Ticket_ID AND bookinginfo.SP_id='$_SESSION[SP_id]'"; 

//$query="SELECT * FROM booker_details WHERE delete_status=1 GROUP BY Ticket_ID ORDER BY booker_id";
$fx=0;
$query="SELECT bookinginfo.Ticket_ID,bookinginfo.travelling_date,bookinginfo.SP_id,bookinginfo.Bus_id,bookinginfo.pay_status,booker_details.Booker_email,booker_details.Booker_name FROM bookinginfo,serviceprovider_info,businfo,booker_details WHERE bookinginfo.Ticket_ID!='' AND  bookinginfo.Ticket_ID=booker_details.Ticket_ID and bookinginfo.pay_status='1' AND bookinginfo.SP_id='$_SESSION[SP_id]'";
...
if(!empty($search_to)){
   $query.=" AND bookinginfo.Bus_id=businfo.Bus_id AND businfo.Bus_tocity=".$search_to;
    ...
    $qry=$query." GROUP BY bookinginfo.Ticket_ID ORDER BY bookinginfo.travelling_date DESC";
	$query.=" GROUP BY bookinginfo.Ticket_ID ORDER BY bookinginfo.auto_id DESC LIMIT $start, $per_page";
...
<table width="100%" border="0" cellspacing="2" cellpadding="5">
  ...
  if(mysql_num_rows($result_pag_data)>0)
  {
    $count=mysql_num_rows($result_pag_data);
		...
		$mn=mysql_fetch_array(mysql_query("select * from booker_details where Ticket_ID='$tik'"));
		...
		$scount12 = mysql_fetch_array(mysql_query("select * from cities where id = '$frm' and del_status!=0"));
		
		$fcity = ucwords($scount12['city_name']) ;
		
		$to = $businfo['Bus_tocity'] ;
		
		$scount13 = mysql_fetch_array(mysql_query("select * from cities where id = '$to' and del_status!=0"));
		...
		$msg.='<tr>			
			...
			<td align="left" nowrap="nowrap">'.date("d-m-Y",strtotime($travelling_date)).'</a></td>
			
			<td align="left" nowrap="nowrap">'.$fcity .'-'. $tcity.'</a></td>
			<td align="left" nowrap="nowrap">'.$fare.'</a></td>
			
			<td align="center"><a href="view_seats.php?ticket_no='.$ticket.'&dat='.$travelling_date.'&fromcity='.$frm.'&tocity='.$to.'&SP_id='.$row['SP_id'].'">'.$scount.'</a></td>
		...
		$opt="c";
		$msg.=''.$status.'</td>
		<td><a href="view_seats.php?ticket_no='.$ticket.'&dat='.$travelling_date.'&fromcity='.$frm.'&tocity='.$to.'&SP_id='.$row['SP_id'].'" onclick=if(confirm(Are you sure to delete this record)){ return true; } else { return false; } >
			Cancel
			</a></td>			
		  </tr>';
    }
