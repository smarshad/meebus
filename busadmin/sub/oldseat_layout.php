...
?>
	...
	<form name="seat_form" id="seat_form" action="sub/seat_layout.php" method="post" onsubmit="return validate_busseats()">
						...
						<tr>
							...
							<td valign="top">						
								<table cellpadding="3" cellspacing="5" border="0" width="100%" bgcolor="#FFFFFF" id="chk_seats">
									...
									<input class="txtbox" type="<?php if(!in_array($r,$seats)){ echo 'text'; } else { echo 'hidden'; }?>" name="<?php echo "seat_".$r ?>" id="<?php echo "seat_".$r ?>" size="2" maxlength="2" <?php if(in_array($r,$seats)) { ?> value="xx" style="background:#666666;" <?php } else {?> value="<?php echo $seat->seat_no; ?>" <?php } ?> onkeydown="return AllowcharNum(event);" />
									 <?php									 
									}
										echo "</tr>";
										}							
