...
$query = "SELECT * FROM smslog where sms_spid='$_SESSION[SP_id]'";
...
$dat6=date("Y-m-d",$dat5); 
  ...
  { 
  $flag=1;
  $query .=" AND (sms_date ='$dat1')"; 
  }
  else if($search =='2')
  {
  $flag=1;
   $query .=" AND (sms_date between '$dat4' and '$dat1')"; 
  }
   else if($search =='3')
  {
  $flag=1;
   $query .=" AND (sms_date between '$dat6' and '$dat1')"; 
  }
  else 
  {
   $query .=""; 
...
$msg .='<div class="data"><table width="100%" border="0" cellspacing="2" cellpadding="5">
  ...
  if(mysql_num_rows($result_pag_data)>0)
  {
		while ($row = mysql_fetch_array($result_pag_data)) 
		{
		   ...
		   $bus_fetch=mysql_fetch_array(mysql_query("select * from businfo where Bus_id='$row[smsbus_id]'"));
		   $bus_name=$bus_fetch['Bus_name'];
          $usertype=$row['sms_usertype'];
		  $userid=$row['sms_userid'];
		  $ticketno=$row['sms_ticket'];
		...
		$msg .= '<tr>
			...
			<td align="left">'.$sms_from.'</td>
			<td align="left">'.$sms_to.'</td>
			...
			<td align="center">
					...
					<a href="javascript:void(0);" onclick="delsms('.$id.')">
...
if(r)
window.location.href="delete_sms.php?smsid="+uid;
}
</script>
