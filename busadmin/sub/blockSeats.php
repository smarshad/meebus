...
if(isset($_REQUEST['status']) && $_REQUEST['status'] != "Choose Date" )
{
...
$query = "SELECT * FROM bookinginfo, serviceprovider_info WHERE bookinginfo.Blocked = '1' and (bookinginfo.travelling_date = '0000-00-00' or bookinginfo.travelling_date = '".$dat."') and bookinginfo.SP_id=".$_SESSION['SP_id']." and serviceprovider_info.SP_id = '".$_SESSION['SP_id']."' " ;
...
<table width="100%" cellspacing="1" cellpadding="1" align="left">
  ...
  if(mysql_num_rows($result_pag_data)>0){ // mysql_num_rows($result_pag_data);
     $chk_busid=0; $chk_date=(string).0;
		...
		$row_SPid=$row['SP_id'];
		$row_date=date("d-m-Y",strtotime($row['travelling_date']));
...
/*		if($dat==''){
		...
		//}
		 ...
		 $n=numberofBlockedSeats($_SESSION['SP_id'],$row_Busid,$row_date)."<br>";
		
		if(numberofBlockedSeats($row_SPid,$row_Busid,$row_date)>0){
		...
		$msg.='
			...
			<!--<a href="block_seats.php?sp_id='.$row_SPid.'&bus_id='.$row_Busid.'&dat='.changedateformat($row_date).'">'.numberofBlockedSeats($row_SPid,$row_Busid,$row_date).'</a>-->
