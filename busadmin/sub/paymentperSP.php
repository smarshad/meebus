...
$date=changedateformat($ex[0]);
$from=$ex[1];
$to=$ex[2];
$status=$ex[3];
$sp_id=$ex[4];

 $query="SELECT * FROM businfo, bookinginfo WHERE bookinginfo.travelling_date='".$date."' AND bookinginfo.SP_id=".$sp_id."  AND bookinginfo.Bus_id= businfo.Bus_id AND cancelledStatus=0 ";
...
$chk_bus=0;  $tot=0;
	while($row=mysql_fetch_array($result_pag_data)){
		...
		$to=$row['Bus_tocity'];
		$type=$row['Bus_type'];
		$ind_fare=get_ind_fare($Bus_id,$date);	
		$tot+=$ind_fare;
...
//echo $tic; 
   ...
   $temp_bal=$temp_bal+$balll;
	 ...
	 $temp_tot=$temp_tot+$sum;
		...
		$msg.='<tr>';
		$msg.='<td align="left"><a href="paymentpertrans.php?sp_id='.$sp_id.'&dat='.$date.'&b_id='.$Bus_id.'">'.$Bus_name.'</a>
		       <td align="left">'.get_bus_type($type).'			   
			   <td align="left">'.$from_to.'
			 <!--  <td align="left">'.$ind_fare.'-->
			    <td align="left">'.$sum.'</td>
				
				<td align="center">'.$tic.'</td>
			      <td align="left">'.$com_per1.'
				 <td align="left">'.$com_per2.'
				  <td align="left">'.$tcom_per1.'
				 <td align="left">'.$tcom_per2.'
				  <td align="left">'.$br.'
				 <td align="left">'.$t.'
				  <td align="left">'.$aincome.'
				   <td align="left">'.$sincome;
	...
	$msg.='<tr bgcolor="#CCCCCC"><td align="right" colspan="3"><b>Total Amount</b></td><td align="center"><b>'.$temp_tot.'</b></td><td align="center"><b>'.$temp_tic.'</b></td><td align="center"></td><td align="center"></td><td align="center"><b>'.$temp_c1.'</b</td><td align="center"><b>'.$temp_c2.'</b</td><td align="center"><b>'.$temp_b.'</b></td><td align="center"><b>'.$temp_tax.'</b></td><td align="center"><b>'.$temp_ai.'</b></td><td align="center"><b>'.$temp_sp.'</b></td></tr>';
}
	else {
            $msg.='<tr>
			<td></td>
