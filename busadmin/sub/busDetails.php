...
$query="SELECT businfo.* , serviceprovider_info.* FROM businfo, serviceprovider_info WHERE businfo.SP_id=".$need_id."  AND  serviceprovider_info.SP_id=".$need_id." AND businfo.SP_id = serviceprovider_info.SP_id AND (".$va." OR  businfo.Bus_name LIKE '%".$str."%')";
  ...
  $g=0;
		while ($row = mysql_fetch_array($result_pag_data)) {
		       ...
		       $from_to=get_city_name($row['Bus_fromcity'])."-".get_city_name($row['Bus_tocity']);
                       $fromcity=$row['Bus_fromcity'];
                       $tocity=$row['Bus_tocity'];
		...
		if($g==0){
           $msg.='<tr><td style="width:115px;"><strong>Service Provider Name:</strong>&nbsp;'.ucfirst($SP_name).'</td>
		   <td style="width:115px;"><a href="addNewBus.php?sp_id='.$_SESSION[SP_id].'" class="ovalbutton"><span><strong>Add New Bus</strong></span></a></td>
		   </tr>';
		   $msg.='<tr>
...
<td align="left"><strong>Assign Agent</strong></td>	
<td align="left"><strong>Disable Date</strong></td>
	...
	<td align="left"><strong>Actions</strong></td>
		...
		$msg.='			
			...
			<td><a href="bus_track.php?id='.$bus_id.'">Track</a></td>
                         <td><a href="ass_agent.php?busid='.$bus_id.'&from='.$fromcity.'&to='.$tocity.'">Assign Agent</a></td>
			<td align="center">
		...
		$msg.='</tr>';
			...
			<td align="center"><span class="err_msg"></span></td>
			 <td align="right"><a href="addNewBus.php?sp_id='.$_SESSION[SP_id].'" class="ovalbutton"><span><strong>Add New Bus</strong></span></a></td>
