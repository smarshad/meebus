...
//echo $boarding_time." - ".$dropping_time;exit;
	...
	$sql_businfo="INSERT INTO `businfo` ( `Bus_id` ,`SP_id` ,`SR_id` ,`Bus_fromcity` ,  `Bus_tocity` ,  `Bus_type` ,  `Bus_structure` ,  `Bus_name` , `Bus_boarding_time` , `Bus_departure_time`,  `Bus_fare` ,  `Bus_adddate` ,  `Bus_seats4SP` ,  `Bus_seats4SA` ,  `Bus_totalseats` ,  `Bus_status`,`luxury_item`,`bus_video`,`conditions`,`active_days`) VALUES (NULL ,  '$SP_id',  '$SR_id',  '$source',  '$destination',  '$busType',  '$seats',  '$busname',  '$boarding_time', '$dropping_time',  '$total',  '$date',  '0',  '0',  '40',  '1','$item','$vimg','$terms','$active_days')";
...
/*	if(form.fare.value == ""){
	...
	var fromboard=document.getElementById('boardingpoint').value;
	
	if(fromboard=='') {
		alert("Please choose from boarding point");
		//document.getElementById('b_0').focus();
		return false;
	}
	
	var toboard=document.getElementById('toboardingpoint').value;
	
	if(toboard=='') {
		alert("Please choose to boarding point");
		//document.getElementById('b_0').focus();
		return false;
	}
...
<!--	<option value="<?php echo $lux_row['lux_id'];?>" <?php if($lux_row['lux_id']==$bus->luxury_item) { ?> selected="selected"; <?php } ?>><?php echo $lux_row['lux_name'];?></option>-->
	...
	<td>Active Days</td>
	<td><input type="text" name="active_days" id="active_days" class="textbox" />	</td>
