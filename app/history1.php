...
//echo $t="select distinct Bus_id from bookinginfo where agent_id='$agent_id' and booked_date='$dat'";
    $sql1=mysql_query("select * from bookinginfo where agent_id='$agent_id' and booked_date='$dat'  group by Ticket_ID order by Ticket_ID desc  ");
while($qry = mysql_fetch_assoc($sql1)){
    $Bus_ids=$qry['Bus_id'];
  //echo "select * from bookinginfo where Bus_id='$Bus_ids' and agent_id='$agent_id' and pay_status='1' and  cancelledStatus='0'  and booked_date='$dat' group by Ticket_ID order by auto_id desc";
  
    $sq2=mysql_query("select * from bookinginfo where Bus_id='$Bus_ids' and agent_id='$agent_id' and pay_status='1' and  cancelledStatus='0'  and booked_date='$dat' group by Ticket_ID order by  Ticket_ID desc");
while($qry1 = mysql_fetch_assoc($sq2)){
$Ticket_ID2=$qry1['Ticket_ID'];
      $sq21=mysql_query("select * from bookinginfo where Bus_id='$Bus_ids' and Ticket_ID='$Ticket_ID2' and agent_id='$agent_id'  and cancelledStatus='0' and booked_date='$dat'  ");
