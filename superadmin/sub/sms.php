...
$dat6=date("Y-m-d",$dat5); 
  ...
  { 
  $flag=1;
  $query .=" where (sms_date ='$dat1')"; 
  }
  else if($search =='2')
  {
  $flag=1;
   $query .=" where (sms_date between '$dat4' and '$dat1')"; 
  }
   else if($search =='3')
  {
  $flag=1;
   $query .=" where (sms_date between '$dat6' and '$dat1')"; 
  }
  else 
  {
   $query .=""; 
...
$msg .='<div class="data"><table width="100%" border="1" cellspacing="2" cellpadding="10">
  ...
  if(mysql_num_rows($result_pag_data)>0)
  {
		while ($row = mysql_fetch_array($result_pag_data)) 
		{
		  ...
		  $transid=$row['sms_trans_no'];
		  //echo smsresponse($transid);exit;
