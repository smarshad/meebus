...
$query="SELECT * FROM booker_details,cancelled_tickets,serviceprovider_info WHERE cancelled_tickets.Ticket_id=booker_details.Ticket_id AND cancelled_tickets.SP_id=serviceprovider_info.SP_id ";
...
<table width="100%" border="0" cellspacing="2" cellpadding="5">
  <tr>
	...
	<th nowrap="nowrap">Refund Amount (RS.)</th>
	<th nowrap="nowrap">Booking Amount (RS.)</th>
  ...
  if(mysql_num_rows($result_pag_data)>0)
  {
    $count=mysql_num_rows($result_pag_data);
		...
		//echo $row['cancelled_status']."dfsd";
		...
		$msg.='<tr>	 
			...
			<a href="ticket_view.php?ticket='.$ticket.'&travel_date='.$travelling_date.'&Bus_id='.$row['Bus_id'].'&SP_id='.$row['SP_id'].'&email='.$bemail.'" class="link1">'.$ticket.'</a></td>	-->
			<td align="left" nowrap="nowrap">
			<a href="cancel_seats.php?ticket_no='.$ticket.'&dat='.$travelling_date.'&fromcity='.$fromcity.'&tocity='.$tocity.'&SP_id='.$row['SP_id'].'" class="link1">'.$ticket.'</a></td>	
			...
			<td align="center">'.$cancelcount.'	</td>
			<td align="center">'.$refund.'</td>
			<td>		
			<a href="cancel_seats.php?ticket_no='.$ticket.'&dat='.$travelling_date.'&fromcity='.$fromcity.'&tocity='.$tocity.'&SP_id='.$row['SP_id'].'" class="link1">View</a></td>
		  </tr>';		 
		}
    }
	else {
