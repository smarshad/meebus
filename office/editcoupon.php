...
if(isset($_REQUEST['coupid'])){

$coupid=$_REQUEST['coupid'];
$sp_id=$_REQUEST['sp_id'];
$bus_id=$_REQUEST['bus_id'];
$coup=mysql_fetch_array(mysql_query("select * from bus_coupons where coup_id='$coupid'"));
?>
<a href="busDetails.php?sp_id=<?php echo $sp_id; ?>"><?php echo get_SP_name($sp_id); ?></a>&nbsp; >> &nbsp;<?php echo get_bus_name($bus_id); ?>&nbsp; >> &nbsp;<strong>Edit Promotional code</strong>
<?php 
if(isset($_REQUEST['editcoup'])){

extract($_POST);
$date_from=date("Y-m-d",strtotime($_REQUEST['date_from']));	
$date_to=date("Y-m-d",strtotime($_REQUEST['date_to']));

	mysql_query("update `bus_coupons` set `coup_perc`='$percent',`coup_date`='$date_from',`coup_date1`='$date_to' where coup_id='$coupid'");
...
<table width="666" align="center">
	...
	<tr>
		...
		<?php echo $coup['coup_unique']; ?>
			<input type="hidden" name="coupon" id="coupon" class="textbox" value="<?php echo $coup['coup_unique']; ?>"/>	
	...
	<tr>
		...
		<td width="345">
			<input type="text" name="percent" id="percent" class="textbox" value="<?php echo $coup['coup_perc']; ?>"/>	
	...
	<tr>
		...
		<td width="345">
			<input type="text" name="date" id="date" readonly="readonly" class="textbox" value="<?php echo date("d-m-Y",strtotime($coup['coup_date'])); ?>"  />	<input type="hidden" name="date_from" id="date_from" />
	...
	<tr>
		...
		<td width="345">
			<input type="text" name="date1" id="date1" readonly="readonly" class="textbox" value="<?php echo date("d-m-Y",strtotime($coup['coup_date1'])); ?>"  /><input type="hidden" name="date_to" id="date_to" />
	...
	<tr>
		...
		<td colspan="2" style="padding-left:150px;"> 
			...
			<!--<input type="submit" name="subbus" value="Add >>" onClick="add_validbus()" />-->
				<input type="submit" name="editcoup" value="Update Coupon >>"  />
	    </td>
