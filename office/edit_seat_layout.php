	  ...
	  for($i=1;$i<=$_REQUEST['seat_count'];$i++)
	  {
		  ...
		  $rcv =$_POST[$rc];
           ...
           $owbuup = "UPDATE businfo SET Bus_totalseats='$cnt', Bus_structure='$bus_seatlayout' WHERE Bus_id='$bid'";
		    mysql_query($owbuup);
		  header("location:edit_multipledesignation.php?busid=".$bid);
...
function chg_structure(strctureID){
	...
	window.location.assign('edit_seat_layout.php?busid='+busid+'&str_id='+strctureID);
...
<fieldset class="table-bor">
  ...
  <table width="382">
    ...
    $owsel3 = "SELECT * FROM bus_structure WHERE bus_id='$bus_id'";   
    $ss=mysql_query($owsel3);
	$tmp='';
	while($bs=mysql_fetch_array($ss)){
		 $tmp = explode(',',$bs['rc']);
         $buss[$tmp[0]][$tmp[1]][$tmp[2]]=$bs['seat_no'];  
        }
//echo "<pre>"; print_r($buss); echo "<pre/>"; exit;  ?>
      <select name="bus_seatlayout"  id="bus_seatlayout"  onchange="chg_structure(this.value)">
          <?php
...
?>
  ...
  <div id="chg_struct">
    ...
    <td><input type="text"  name="s<?php echo $seatCount; ?>" style="width:<?php echo $width; ?>;" id="s<?php echo $seatCount; ?>" value="<?php if(!isset($_GET['str_id'])) { echo $buss['UL'][$up_lr][$up_lc]; } ?>">
	...
	<tr>
  		<?php  for($up_lc=1; $up_lc<=$upper_right_col; $up_lc++) { 
		if($upper_right_type=='sleeper') { $width ='50px'; } else {  $width ='20px'; } ?>
    <td><input type="text" name="s<?php echo $seatCount; ?>" style="width:<?php echo $width; ?>;" id="s<?php echo $seatCount; ?>" value="<?php if(!isset($_GET['str_id'])) { echo $buss['UR'][$up_lr][$up_lc]; } ?>">
...
<tr>
    ...
    <td><input type="text" name="s<?php echo $seatCount; ?>" style="width:<?php echo $width; ?>;" id="s<?php echo $seatCount; ?>" value="<?php if(!isset($_GET['str_id'])) { echo $buss['LL'][$up_lr][$up_lc]; } ?>">
	...
	<tr>
  		<?php  for($up_lc=1; $up_lc<=$lower_right_col; $up_lc++) { 
		if($lower_right_type=='sleeper') { $width ='50px'; } else {  $width ='20px'; } ?>
    <td><input type="text" name="s<?php echo $seatCount; ?>" style="width:<?php echo $width; ?>;" id="s<?php echo $seatCount; ?>" value="<?php if(!isset($_GET['str_id'])) { echo $buss['LR'][$up_lr][$up_lc]; } ?>">
