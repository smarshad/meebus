...
$a=''; $b='';
	...
	$sql_businfo="UPDATE `businfo` SET `Bus_fromcity` = '$source' ,  `Bus_tocity` = '$destination' ,  `Bus_type` = '$busType' , `Bus_name` = '$busname' ,  `Bus_boarding_time` = '$boarding_time' ,`Bus_departure_time`='$drop_time', `Bus_fare` = '$total' , `Bus_totalseats` = '$seats',`luxury_item`='$item',`bus_video`='$video',`bus_image`='$bus_img_small' WHERE `Bus_id` = '$bus_id' ";
...
<table>
	...
	<tr><td colspan="4"><br /></td></tr>
	<?php 
	$drop=explode("-",$bus->Bus_departure_time);
	echo $drop[0];
	echo $drop[1];
	?>
	...
	<tr>
		...
		<td colspan="3" align="left">
			...
			<table id="dropping_tbl">
				<tr>
					<td width="145">
					<input type="text" name="dropping1" id="dropping1" value="<?php echo $drop[0] ; ?>" class="textbox" />						
				  </td>
					
					<td width="163">
						<input type="text" name="to_time1" id="to_time1" value="<?php echo $drop[1] ; ?>" class="textbox" value="choose time" onfocus="time_show(this.id);" />
				  </td>
					
					<td width="10">&nbsp;</td>
					
					<td width="21"><img src="images/plus_icon.gif" border="0" onclick="add_dropping();" /></td>
					
					<td width="21"><img src="images/minus_icon.png" border="0" onclick="del_dropping();" /></td>
	...
	<td>Bus Videos</td>
	<td><input type="text" name="video" id="video" value="<?php echo $bus->bus_video; ?>"><br><span>EX: http://www.youtube.com/watch?v=u1zgFlCw8Aw</span>      						</td>
