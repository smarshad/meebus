...
$flag=0;
$query = "SELECT * FROM bus_luxitem";
$query .=" WHERE ";
if(isset($_REQUEST['search']) && $_REQUEST['search']!=''){
  $search=mysql_real_escape_string($_REQUEST['search']);
  $flag=1;
  
  $query .=" `lux_name` LIKE '%".$search."%'"; 
  }
...
$query .= " ORDER BY lux_name LIMIT $start, $per_page";
...
$msg .='<div class="data">&nbsp;<br><table border="0" width="50%" cellspacing="2" cellpadding="5" align="center">
  ...
  $g=0;
		...
		$lux_id=$row['lux_id'];
		$lux_name=$row['lux_name'];
		$status=$row['lux_status'];
		...
		$msg.='
		<tr>
			<td align="left">'.$lux_name.'<input type="hidden" id="'.$lux_id.'" value="'.$lux_name.'"></td>
			<td align="left" nowrap="nowrap">
			<a href="javascript:void(0);" onclick="changelux('.$lux_id.','.$s.')" title="Click to '.$title.'">
			<img src="'.$src.'" border="0" />
			</a>
						
			<a href="javascript:void(0);" onclick="editluxfun('.$lux_id.')" title="Edit">
			...
			<a href="javascript:void(0);" onclick="dellux('.$lux_id.')" title="Delete">
			<img src="../images/delete.png" border="0" />
			</a>
            ...
            $msg.='<tr>
			<td colspan="2" align="center"><span class="err_msg">No Found Records.</span></td>
		  </tr>';	
	}	
	$msg.= "</table></div>" ;
...
$query_pag_num = "SELECT COUNT(*) AS count FROM bus_luxitem where lux_status!=2 ";

if(isset($_REQUEST['search']) && $_REQUEST['search']!=''){
  $search=mysql_real_escape_string($_REQUEST['search']);
  $query_pag_num .=" WHERE ";
  $query_pag_num .=" `lux_name` LIKE '%".$search."%' ";  
  }
