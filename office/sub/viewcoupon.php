...
//echo $need_id=$_REQUEST['sp_id']; exit;
 $coupid=$ex[0]; 
$status=$ex[1];
$ssp_id=$ex[2]; 
$bus_idd=$ex[3];
...
$query="SELECT businfo.* , bus_coupons.* FROM businfo, bus_coupons WHERE businfo.SP_id=".$ssp_id."  AND bus_coupons.coup_bus=".$bus_idd."  AND businfo.Bus_id=bus_coupons.coup_bus AND businfo.SP_id = bus_coupons.coup_sp AND (".$va.")";
...
$msg .='<div class="data" style="width:100%;"><table border="0" width="100%" cellspacing="2" cellpadding="5" align="left">';
  ...
  $g=0;
		...
		if($g==0){
		   ...
		   <td style="width:140px;" colspan="4"><a href="addnewcoupon.php?sp_id='.$ssp_id.'&busid='.$bus_idd.'" class="ovalbutton"><span><strong>Add Coupon</strong></span></a></td>
		   </tr>';
		   $msg.='<tr>
	...
	<td></td>
		...
		$msg.='</tr>';
			...
			<td><span class="err_msg"></span></td>
			 <td><a href="addnewcoupon.php?sp_id='.$ssp_id.'&busid='.$bus_idd.'" class="ovalbutton"><span><strong>Add Coupon</strong></span></a></td>
