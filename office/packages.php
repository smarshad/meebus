...
if($aws!='')
{
	$data=array($offid,$aws);
	$res=$obj->getCargoDetailsByAWS($data);
	
}
else if($destination!='' && $type!='')
{
	$data=array($offid,$destination,$type);
	$res=$obj->getCargoDetailsByDestAndType($data);
}
else if($destination!='')
{
	$data=array($offid,$destination,$type);
	$res=$obj->getCargoDetailsByDest($data);
}
else if($type!='')
{
	$data=array($offid,$destination,$type);
	$res=$obj->getCargoDetailsByType($data);
}
else
{
	$data=array($offid);
	$res=$obj->getCargoDetails($data);
}

?>
<div class="page-scroll">
                            <table cellpadding="0" class="table-border" cellspacing="0" border="0" width="100%">
                                <thead>
                                    <tr>
