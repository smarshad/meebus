...
$c=''; $d='';
	...
	$sql_businfo="UPDATE `businfo` SET `Bus_fromcity` = '$source' ,  `Bus_tocity` = '$destination' ,  `Bus_type` = '$busType' , `Bus_name` = '$busname' ,  `Bus_boarding_time` = '$boarding_time' ,`Bus_departure_time`='$dropping_time', `Bus_fare` = '$total' , `Bus_totalseats` = '$seats',`luxury_item`='$item',`bus_video`='$vimg',`bus_image`='$bus_img_small',`active_days`='$active_days',`bus_promo`='$bus_promo',conditions='$terms' WHERE `Bus_id` = '$bus_id' ";
...
<table>
	...
	<tr><td colspan="4"><br /></td></tr>
		...
		<td></td><td></td><td></td>
		<td>
		<strong><a href="edit_seat_layout.php?busid=<?php echo $bus_id; ?>">Edit Bus Structure</a></strong>
		</td>
		</tr>
		<tr>
		<td></td>
	...
	<tr>
		...
		<td width="183"><a href="edit_seat_layout.php?busid=<?php echo $bus_id; ?>" class="ovalbutton"><span><strong>
		<?php
...
<!--	<option value="<?php echo $lux_row['lux_id'];?>" <?php if($lux_row['lux_id']==$bus->luxury_item) { ?> selected="selected"; <?php } ?>><?php echo $lux_row['lux_name'];?></option>-->
	<div><input type="checkbox"  name="lux_item[]" id="lux_item[]" value="<?php echo $lux_row['lux_id'] ?>" <?php if(in_array($lux_row['lux_id'],$check)) { echo "checked='checked'";   } ?>/><span><?php echo $lux_row['lux_name'];?></span></div>
	...
	<td>Active Days</td>
	<td><input type="text" name="active_days" id="active_days" class="textbox" value="<?php echo $bus->active_days; ?>" />	</td>
