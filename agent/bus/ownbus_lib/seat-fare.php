...
$fareDetail=$obj->getOwnBusFare(array($bus_id));
$fareDetail=$fareDetail[0];
$baseFare=$fareDetail['base_fare'];
$serviceCharge=$fareDetail['serviceCharge'];
$serviceTax=$fareDetail['serviceTax'];
...
//$seatCount=count($seat);
 ...
 {
	 $fare=$obj->agentMarkup($baseFare,$agent_markup,$admin_markup);
	 ...
	 $_SESSION['agent']['bus']['netfare']=$_SESSION['agent']['bus']['netfare']-$serviceCharge-$serviceTax;
	 $_SESSION['agent']['bus']['agent_markup']-=$agent_markup;
	 $agent_comm=(($seat['fare']+$admin_markup)*$agent_commission)/100;
 ...
 { 
	if(count($_SESSION['agent']['bus']['seat']['seatName'])<6)
	 ...
	 {	 $agent_comm=(($seat['fare']+$admin_markup)*$agent_commission)/100;
		 ...
		 $_SESSION['agent']['bus']['commission']+=$agent_comm;
		 $_SESSION['agent']['bus']['seat']['fare'][$seat]=$baseFare;
		 $_SESSION['agent']['bus']['seat']['agent_fare'][$seat]=$fare;
		 $_SESSION['agent']['bus']['seat']['seatName'][$seat]=$seat;
		 $_SESSION['agent']['bus']['seat']['ladies'][$seat]=$seat['ladiesSeat'];
		 $_SESSION['agent']['bus']['netfare']=$_SESSION['agent']['bus']['netfare']+$fare;
		 $_SESSION['agent']['bus']['netfare']=$_SESSION['agent']['bus']['netfare']+$serviceCharge+$serviceTax;
		 $unselect=1;
	 }
...
<input type="radio" id="amount" name="amount" value="wallet" />wallet</label>
<label>
	<a href="payment_gateway.php"><input type="radio" id="amount" name="amount" value="paymentgateway" />Payment Gateway</a>
	</label>
