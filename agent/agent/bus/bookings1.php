...
//$_SESSION['FILL']['PASSENGER'] 	= explode('^',$tmp_pname);
//$_SESSION['FILL']['AGE'] 		= explode('^',$tmp_page);
//$_SESSION['FILL']['GENDER'] 	= explode('^',$tmp_pgender);
...
//if($_SESSION['agent']['log']['id']==849) { echo "<pre>"; print_r($response_oneway); echo "<pre/>"; exit; }

if($response_oneway!='')
{
  //echo $response_oneway; 
  $error_logs= "Page : booking.php,<br/> Blocking Respons GET From API Successfully ";	
  $error_logs_detail=array($_SESSION['agent']['log']['id'],$_SESSION['agent']['log']['agent_name'],$_SESSION['agent']['log']['agency_name'],time(),date('d-m-Y h:i:s A',time()),$error_logs);
  //echo '<pre>'; print_r($error_logs_detail); echo '</pre>'; 
...
else 
{
		...
		$error_logs= "<br/>Pagen Name : booking.php <br/>".$response_oneway['response_oneway'];
		$error_logs_detail=array($_SESSION['agent']['log']['id'],$_SESSION['agent']['log']['agent_name'],$_SESSION['agent']['log']['agency_name'],time(),date('d-m-Y h:i:s A',time()),$error_logs);
		$update_error_logs=$obj->error_logs($error_logs_detail);	
