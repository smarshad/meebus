...
foreach($ticket as $t)
{
	...
	if($t['id']>=4480)
	{
		...
		if(isset($cancel_data) && $cancel_data!='')
		{
			...
			foreach($rec as $rec1)
			{
				$Lessfare = $Lessfare+$passenger_fare[$ia];
				$ia++;
			}
				
		}
		//echo 'Lessfare : '.$Lessfare;
	...
	if(isset($cancel_data) && $cancel_data!='') 
		{
			...
			foreach($cancel_data as $test)
			{ 
				//echo $ib;
				if($test==$passenger['seat'][$i])
				{
					echo $test;
					//$i++;
				}	
				if($test!=$passenger['seat'][$i] && isset($passenger["name"][$i]))
				{
					$firstName = $passenger["name"][0];
					if($passenger["sex"][$i]=='Male'){ $mrms = "Mr. "; } else  { $mrms = "Ms. "; }
					...
					<td align="center" style="display:none;">'.$passenger_fare[$i].'</td>
					</tr>';
...
echo $ticket_html.=$ticket_htmla; exit;
$ticket_html.=$ticket_htmla.'</tbody></table></td></tr><tr><td align="left" style="font-size:10px;padding-top:0px;border-bottom:solid #b2b2b2 1px;text-align:left" colspan="2"></td></tr><tr><td style="color:#7a7a7a; font-size:10px; border-bottom:1px solid #E0E0E0; " colspan="2" >'.$cnclDatas.'</td></tr><tr><td colspan="2"><strong>Agent Info</strong><table width=100% border=0><tr><td width=15%>Agnecy Name</td><td width=35%>'.$_SESSION['agent']['log']['agency_name'].'</td><td width=50% align="right">Customer Support No : +91 95669 43649</td></tr><tr><td>Mobile</td><td>'.$_SESSION['agent']['log']['mobile'].'</td>  <td>&nbsp;</td></tr><tr><td>Email</td><td>'.$_SESSION['agent']['log']['email'].'</td>  <td>&nbsp;</td></tr><tr><td>&nbsp;</td><td colspan="2">&nbsp;</td></tr></table></td></tr></table><br/></td></tr></tbody></table></td></tr></tbody></table><div class="yj6qo"></div><div class="adL"></div></div></div></div>'; 
//echo $ticket_html;
?>
</div>
<?php
}  
echo $_SESSION['agent']['bus']['ticket']=$ticket_html;
?>
