...
function cancelString($inventoryIdC,$passenger_seat1,$tin,$email)
{
    $_SESSION['gs_pass_urlqruy'] = $cancelRequest = "CancelBusTicket?referenceNo=".$inventoryIdC."&seatNos=".$passenger_seat1."&emailId=".$email;
    $CancelTicket1 = cancelTicket($cancelRequest);
	return $CancelTicket1;
}
...
$cemailId =$cancel_detail[0]['emailId'];
$inventoryIdC = $cancel_detail[0]['inventoryId'];
$cancel_cou=0;
...
if($cancel_detail!=0)
{
	...
	$_SESSION['gs_pass_cancel'] = $inventoryIdC.' -$- '.$cancelled_seat.' -$- '.$tin.' -$- '.$cemailId;
	
	$_SESSION['gs_get_cancel'] = $CancelTicket=cancelString($inventoryIdC,$cancelled_seat,$tin,$cemailId);
	//print_r($CancelTicket);
	unset($_SESSION['cancel']['refund']);
	unset($_SESSION['cancel']['cancel_charge']);
	unset($_SESSION['cancel']['tin']);
	$_SESSION['cancel']['refund']			=	$CancelTicket['TotalRefundAmount'];
	$_SESSION['cancel']['cancel_charge']	=	$CancelTicket['CancellationCharges'];
	if($CancelTicket['TicketNumber']=='')
	{
	$_SESSION['cancel']['tin']				=$tin;	}
	else
	{
	$_SESSION['cancel']['tin']				=	$CancelTicket['TicketNumber'];
	}
}
//echo $_SESSION['GS_GET_CANCEL']; 
//print_r($_SESSION['GS_GET_CANCEL']);
...
$amount=$user_refund;
if($CancelTicket['BookingStatus']==5)
{
	...
	//echo '<pre>'; print_r($insert_cancel);  echo '</pre>'; 
