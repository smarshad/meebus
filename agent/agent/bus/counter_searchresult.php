<?php 
   ...
   //echo $agentid;
   $da= $obj-> getSPID(array($agentid,2));
   //print_r($da); exit;
          foreach($da as $r2)
   {
      $busid1=$r2['bus_id'];
   $bussql_1=$obj->getOunbusList1(array($busid1,$origin_id,$destination_id,1));
 // echo "<pre>";
//  print_r($bussql_1);
//   echo "</pre>";
   $cnt=count($bussql_1);
                                 ...
                                 <div id="users">
                                    <table id="bus">
                                       ...
                                       <tbody class="list">
                                          ...
                                          <?php }else {
                                             if(isset($results['availableTrips'][0])){
                                          ...
                                          <tr class="fitter bus_filt <?php echo str_replace(')','',str_replace('(','',str_replace('.','_',str_replace('&','_',str_replace('/','_',str_replace(' ','_',$bus_result['travels'])))))); ?> <?php echo str_replace('+','_',str_replace(')','_',str_replace('(','_',str_replace('&','_',str_replace('/','_',str_replace(' ','_',$bus_result['busType'])))))); ?>  <?php if(isset($bus_result['boardingTimes'][0]) && $bus_result['boardingTimes'][0]!='') { for($tm11=0; $tm11<count($bus_result['boardingTimes']); $tm11++) { echo ' '.str_replace('&','_',str_replace('/','_',str_replace(' ','_',$bus_result['boardingTimes'][$tm11]['bpName']))); } } else { echo ' '.str_replace('&','_',str_replace('/','_',str_replace(' ','_',$bus_result['boardingTimes']['bpName']))); } ?>  <?php if(isset($bus_result['droppingTimes'][0]) && $bus_result['droppingTimes'][0]!='') { for($tm21=0; $tm21<count($bus_result['droppingTimes']); $tm21++) { echo ' '.str_replace('&','_',str_replace('/','_',str_replace(' ','_',$bus_result['droppingTimes'][$tm21]['bpName']))); } } else { echo ' '.str_replace('&','_',str_replace('/','_',str_replace(' ','_',$bus_result['droppingTimes']['bpName']))); } ?>">
                                           ...
                                           <?php     if(strpos($bus_data['departureTime'],'am')==false && strpos($bus_data['departureTime'],'pm')==false)
   	$Depart_time = date("g:i A", strtotime(floor($bus_data['departureTime'] / 60) . ":" . $bus_data['departureTime'] % 60)); 
	else
	$Depart_time = $bus_data['departureTime']; ?>
    
    
                                                <span class="sub-title">
												<?php     
												if(strpos($bus_result['departureTime'],'am')==false && strpos($busbus_result_data['departureTime'],'pm')==false)
   	$Depart_time = date("g:i A", strtotime(floor($bus_data['departureTime'] / 60) . ":" . $bus_result['departureTime'] % 60)); 
	else
	$Depart_time = $bus_result['departureTime'];
												 //  if (strpos($bus_result['departureTime'], 'AM') == false && strpos($bus_result['departureTime'], 'PM') == false) {
...
//                                                   }
                                             ...
                                             <td class="sm_Arrival">
                                                <span style="display:none;">
												<?php if(strpos($bus_result['arrivalTime'],'am')==false && strpos($bus_result['arrivalTime'],'pm')==false)
	{
