...
$error_logs.= "Page : View_ticket.php ,<br/>POST Value :".implode('^',$_POST)."<br/>GET Value :".implode('^',$_GET)."<br/>Session Value : Common URL : ".$_SESSION['common']['url']."<br/> Page Name : ".$_SESSION['common']['pagename']."<br/> Agent ID : ".$_SESSION['agent']['log']['id'];
 ...
 <div class="span9" id="content">
                    	<div class="row-fluid">                        
                            ...
                            <div class="navbar navbar-inner block-header">
                                <div class="muted" align="center"><b>" <?php echo $_GET['ticket']; ?> "</b> This Ticket May Be Canceled / Not Valid </div>
                            </div>
                            </div>
                            </div>
                            </div>
...
foreach($ticket as $t)
{
	...
	$_SESSION['agent']['bus']['user_mail'] = $emailId=$t['emailId'];
	...
	if($t['id']>=4480)
	{
		...
		if(isset($t['cancel_data']) && $t['cancel_data']!=''){
		$cancel_data = explode('^',$t['cancel_data']); }
		else { $cancel_data='';	}
		if(isset($cancel_data) && $cancel_data!=''){
		foreach($cancel_data as $cancel_data1){
		$rec[]= array_search($cancel_data1,$passenger['seat']); }
		$ia=0; foreach($rec as $rec1){ 
		$Lessfare = $Lessfare+$passenger_fare[$ia]; $ia++; } }
	}
	else
	{
		...
		if(isset($t['travelsPhoneNbr']) && $t['travelsPhoneNbr']!='') { 
		$boardingcontact="Contact No : ".$t['travelsPhoneNbr']; } else { $boardingcontact=''; }
...
for($i=0; $i<count($passenger['name']); $i++ )
{
	if(isset($cancel_data) && $cancel_data!='') 
		{
			if(!in_array($passenger["seat"][$i],$cancel_data))
			{
				$firstName = $passenger["name"][0];
				if($passenger["sex"][$i]=='Male'){ $mrms = "Mr. "; }
				else { $mrms = "Ms. "; }
				$ticket_htmla.='<tr  height="30" style="color:#5f5f5f">
...
$ticket_html.=$ticket_htmla.'</tbody></table></td></tr><tr><td style="color:#7a7a7a; font-size:10px; border-bottom:1px solid #E0E0E0; " colspan="2" >'.$cnclDatas.'</td></tr>';
...
<table width=100% border=0><tr><td width=15%>Agency Name</td><td width=35%>'.strtoupper($_SESSION['agent']['log']['agency_name']).'</td><td width=50% align="right">Customer Support No : +91 95669 43649</td></tr><tr><td>Mobile</td><td>'.$_SESSION['agent']['log']['mobile'].'</td><td>&nbsp;</td></tr>
<tr><td>Email</td><td>'.$_SESSION['agent']['log']['email'].'</td><td>&nbsp;</td></tr></table></td></tr></table><br/></td></tr></tbody></table></td></tr></tbody></table><div class="yj6qo"></div><div class="adL"></div></div></div></div>'; 
echo $ticket_html;
?>
</div>
<?php
...
$(document).ready(function() {
    $('#email').click(function () {
        $.post( "sendmail.php", function( data ) { 	alert(data); });
    });
});
</script>
